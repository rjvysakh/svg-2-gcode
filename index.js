...
async function convertSvgToGcode(
  svgIn,
  { laserOffSpeed, laserOnSpeed, laserIntensity }
) {
  if (!svgIn) {
    throw new Error("Empty svg path!");
  }
  if (!laserOffSpeed || !laserOnSpeed || !laserIntensity) {
    throw new Error("Enter Fields to continue!");
  }
...
function loadfile(svgString) {
  ...
  var gcodeOutput =  generategcode(
    ...
    0, //input safez
    laserOffMovementSpeed, // movespeed (200) /// F2000  movement when laser off (number * 50.19 for getting F<value>)
...
function cleansvg() {
  ...
  if (svgin.includes("<defs")) {
    ...
    svgin =
      svgin.substring(0, sp - 1) + svgin.substring(ep + 1, svgin.length - 1);
  }
  lines = svgin.split("\n");
...
function drawpatharc(sx, sy, ttrx, ttry, txrot, aflag, sflag, ex, ey, tmat) {
  ...
  var currpx =
    (Math.cos(xrot) * (sx - ex)) / 2.0 + (Math.sin(xrot) * (sy - ey)) / 2.0;
  var currpy =
    (-Math.sin(xrot) * (sx - ex)) / 2.0 + (Math.cos(xrot) * (sy - ey)) / 2.0;
  var l =
  ...
  for (cang = 0; cang <= sweepsteps; cang++) {
    ...
    my += cy;
    if (
      dist(lastx, lasty, mx, my) > arcdist ||
      cang === 0 ||
      cang === sweepsteps
    ) {
      xy = dotransforms(mx, my, tmat);
...
function gettransform(trans) {
  ...
  if (trans !== null) {
    ...
    if (pos != -1) {
    ...
    } // Either matrix or separate operations
    else {
      ...
      mat = [
        s[0] * Math.cos(r[0] * torad),
        s[1] * Math.sin(r[0] * torad),
        -s[0] * Math.sin(r[0] * torad),
        s[1] * Math.cos(r[0] * torad),
        (-r[1] * Math.cos(r[0] * torad) +
          r[2] * Math.sin(r[0] * torad) +
          r[1]) *
          s[0] +
          t[0],
        (-r[1] * Math.sin(r[0] * torad) -
          r[2] * Math.cos(r[0] * torad) +
          r[2]) *
          s[1] +
          t[1],
      ];
...
function generategcode(
  ...
  if (validated === false) {
    ...
    throw new Error("Error found with values");
  }
...
function makecnccommentheader() {
  ...
  gcode +=
    ...
    "mm/min moving\r\n";
  if (cncmode === 0)
    gcode += "; Maximum cut depth: " + getmaxcutdepth() + "mm\r\n";
...
function getmaxfeedrate() {
  ...
  for (c = 0; c < commands.length; c++)
    if (commands[c][0] === "L" && commands[c][6] === 1)
      if (commands[c][8] !== 0) if (commands[c][4] > fr) fr = commands[c][4];
  return fr;
}
//max cut depth
function getmaxcutdepth() {
  ...
  for (c = 0; c < commands.length; c++)
    if (commands[c][0] === "L" && commands[c][6] === 1)
      if (commands[c][8] !== 0) if (commands[c][2] > mc) mc = commands[c][2];
  return mc;
}
/// start gcode
...
function getdefaults() {
  ...
  defaultfeed = laserOnMovementSpeed; ///F600 values for laser on
  defaultspeed = laserIntensityValue; //S1000
  defaultcut = 1;
  defaultpasses = 1;
  defaultgroup = 0;
  defaultbottomprofile = 0;
...
function makecnctext_pathbypath() {
  ...
  switch (originpos) {
    ...
    case 3:
      ...
      break; // Top right
    case 4:
      originx = cutterwidth / 2;
      originy = cutterheight / 2;
      break; // Middle
  }
  lastx = 0;
  lasty = 0;
      ...
      if (commands[c][0] === "L" && commands[c][6] === 1) {
        ...
        if (cgroup === group) {
          ...
          for (pass = 0; pass < cpasses; pass++) {
            ...
            if (cncmode === 1 && commands[c][11] !== 1)
              gcode += "M5 ; Turn off the LASER\r\n";
          }
          if (cncmode === 0 && commands[c][11] === 1) {
            lastz = 0;
            gcode += "G90 ; Set absolute coordinate mode\r\n";
...
function maketoolpath(path, cutdepth, totaldepth, sfx, sfy, cutterheight) {
  ...
  for (c = 0; c < pts.length; c++)
    if (pts[c][2] > cutdepth) pts[c][2] = cutdepth;
